...
- [2026-08-10] Shell `trap … EXIT` REPLACES the prior trap — each later trap must re-list every earlier cleanup PID or those processes orphan past exit and squat their ports for the next run (e2e port uniqueness itself is now lint-enforced in repo_hygiene). And `pkill -f <pattern>` matches the calling shell's own command line — use `pkill -x <name>`.
<!-- merge0:lessons:end -->
